...
static esp_err_t ble_start(void)
{
    ...
    // The ESP32-C3 BLE controller is not implemented by QEMU. Keep the demo
    // in a virtual advertising state; the emulator transport supplies HCI
    // frames without initializing NimBLE's hardware controller.
    s_initialized = true;
    s_start_requested = true;
    s_state = BLE_DEMO_ADVERTISING;
    return ESP_OK;
#else
    if (s_initialized) {
...
#if CONFIG_AI_PASSPORT_QEMU
        lv_label_set_text(s_status, "VIRTUAL BLE HCI\n\nName: FoloPassport\n\nQEMU transport ready\n\nNo radio hardware");
#else
