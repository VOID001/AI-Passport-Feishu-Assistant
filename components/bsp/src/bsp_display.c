...
esp_err_t bsp_display_init(void) {
    ...
    esp_err_t e = esp_lcd_new_rgb_qemu(&qemu_cfg, &s_panel);
    if (e != ESP_OK) {
        ESP_LOGE(TAG, "QEMU RGB panel 初始化失败: %s", esp_err_to_name(e));
        return e;
    }
    ...
    if (e != ESP_OK) return e;
    ESP_LOGI(TAG, "QEMU RGB 显示就绪 %dx%d", BSP_LCD_W, BSP_LCD_H);
    return ESP_OK;
#else
