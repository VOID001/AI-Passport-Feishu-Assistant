...
#if CONFIG_AI_PASSPORT_QEMU
    // QEMU does not emulate the ESP32-C3 ADC conversion event used by the
    // physical three-button resistor ladder. The emulator injects keys through
    // its virtual input backend instead of starting the ADC button driver.
    ESP_LOGI(TAG, "QEMU 虚拟按键后端就绪");
    return ESP_OK;
#else
...
void bsp_button_inject(bsp_btn_t btn, bsp_btn_ev_t ev)
{
#if CONFIG_AI_PASSPORT_QEMU
    if (btn < BSP_BTN_UP || btn >= BSP_BTN_COUNT || !s_cb) return;
    s_cb(btn, ev, s_user);
#else
    (void)btn;
    (void)ev;
