...
```bash
make test          # 仓库检查 + host tests
make build-device  # 生产 ESP-IDF 构建 + 合并镜像验证
make check         # 完整门禁
make run-qemu      # 写入 QEMU 虚拟 Flash 并验证启动
```

交付前，最终生产固件构建后必须执行 `make run-qemu`。在 QEMU 将镜像写入虚拟
8 MB Flash 且固件成功启动前，不得将该发布候选视为已验证。
