...
1. **Host 门禁**：先运行 `make test`，覆盖仓库检查以及全部 C、Python 和
   Node.js host tests。
2. **ccache 缓存恢复**：使用 `actions/cache` 缓存编译中间产物
   （`.ccache`），二次编译大幅提速。缓存 key 含 ref 与 commit SHA；缓存保留
   时间以仓库的 GitHub Actions 设置为准。
3. **编译与验证**（ESP-IDF 5.5.3 / esp32c3）：运行与本地相同的
   `./tools/validate.sh --firmware`。脚本使用 `sdkconfig.defaults` 和
   `partitions.csv` 构建固件，再执行 `idf.py merge-bin`。
4. **验证完整固件**：脚本逐字节确认 bootloader、partition-table 和 app 位于
   `0x0`、`0x8000` 和 `0x10000`，确认 `flash_args` 使用 8 MB Flash，并完整
   检查小程序 BLE 兼容契约，最后输出
   `build/FoloToy-AI-Passport-full.bin`。
5. **上传 artifact**：每次成功构建都上传
   `FoloToy-AI-Passport-full.bin`。普通分支只有从该分支手动运行
   `workflow_dispatch` 才会构建；普通 push 不触发。
6. **发布 tag**：tag 构建完成后，独立 release job 下载上述 artifact，并创建
   GitHub Release。
