...
| NVS | `demo_radio.c` | 为 NimBLE 一次性准备；初始化失败时不得擦除无关 NVS 数据。 |
| Bluetooth LE 协议栈 | BLE 应用页面 | 按需启动，退出时释放。 |
...
NimBLE 和 light/deep sleep 直接使用 ESP-IDF API，不属于板级 BSP。
`demo_radio.c` 负责准备共享 NVS 并执行一次性旧凭证迁移。BLE 页面进入时初始化
无线栈，退出时停止并释放；不会通过擦除整个 NVS 分区来掩盖初始化错误。
deep sleep 会按 ESP32-C3 语义重启应用，示例用 RTC slow memory 记录唤醒次数。
...
| BLE 第二次进入失败 | 退出页时是否已停止并 deinit radio stack，NVS 是否只初始化一次 |
