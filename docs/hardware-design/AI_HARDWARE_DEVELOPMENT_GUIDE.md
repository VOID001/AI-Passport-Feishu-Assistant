...
| NVS | `demo_radio.c` | Prepared once for NimBLE; do not erase unrelated NVS data on initialization errors. |
| Bluetooth LE stack | BLE application pages | Initialized on demand and deinitialized on exit. |
...
NimBLE and sleep use ESP-IDF directly rather than the BSP. `demo_radio.c`
prepares shared NVS and performs the one-time legacy credential migration. The
Bluetooth page allocates its radio stack on entry and stops/deinitializes it on
exit. Do not erase the whole NVS partition to hide initialization errors. Deep
sleep restarts the application and the demo uses RTC slow memory for the wake
counter.
...
| BLE fails on second entry | stack stop/deinit and one-time NVS setup |
