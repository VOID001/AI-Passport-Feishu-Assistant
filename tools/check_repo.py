...
FORBIDDEN_TRACKED_SUFFIXES = {".bin", ".ppm"}
FORBIDDEN_ROOT_MEDIA_PREFIXES = (
    ".audio-stutter-",
    ".down-",
    ".node-click-",
    ".pulse-",
...
def check_release_artifacts(errors: list[str]) -> None:
    ...
    for name in result.stdout.splitlines():
        ...
        if name.startswith(FORBIDDEN_TRACKED_PREFIXES):
            errors.append(f"{name}: unreviewed guide-site artifact must not be tracked")
        if path.suffix.lower() in FORBIDDEN_TRACKED_SUFFIXES:
            errors.append(f"{name}: generated binary must not be tracked")
        if len(path.parts) == 1 and path.name.startswith(
            FORBIDDEN_ROOT_MEDIA_PREFIXES
        ):
            errors.append(f"{name}: root debug media must not be tracked")
...
def check_npm_registries(errors: list[str]) -> None:
    def visit(value: object, path: Path) -> None:
        if isinstance(value, dict):
            ...
            if isinstance(resolved, str) and not resolved.startswith(
                ...
                errors.append(
                    f"{path.relative_to(ROOT)}: non-public npm dependency source"
                )
    ...
    for path in git_files():
        if path.name != "package-lock.json" or not path.is_file():
            continue
        try:
            visit(json.loads(path.read_text(encoding="utf-8")), path)
        except (json.JSONDecodeError, OSError) as error:
            errors.append(f"{path.relative_to(ROOT)}: invalid npm lockfile ({error})")
